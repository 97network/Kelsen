...
contract generalAssemblyProcedure is Procedure{
    ...
    function addOrdreDuJour(address payable _targetOrgan, address payable _contractToAdd, address payable _contractToRemove, bytes32 _ipfsHash, uint8 _hash_function, uint8 _size, bool _canAdd, bool _canDelete, bool _canDeposit, bool _canSpend, uint _propositionType)
    public
    {
        require(isAssemblyOpenedForProposition);
        procedureLibrary.isAllowed(linkedOrgans.firstOrganAddress);
        // Instanciating the proposition adding procedure
        voteOnAdminsAndMastersProcedure propositionProcedure = voteOnAdminsAndMastersProcedure(linkedOrgans.secondOrganAddress);
        propositionProcedure.createProposition(_targetOrgan, _contractToAdd, _contractToRemove, _ipfsHash, _hash_function, _size, _canAdd, _canDelete, _canDeposit, _canSpend, _propositionType);
    }
